...
camera_mode = True  # True: Move camera; False: Rotate object
...
def render(time):
    ...
    if camera_mode:
        ...
        # Dynamic "up" vector for full Z-axis rotation
        upx = -np.sin(phi * np.pi / 180) * np.sin(theta * np.pi / 180)
        upy = np.cos(phi * np.pi / 180)
        upz = -np.sin(phi * np.pi / 180) * np.cos(theta * np.pi / 180)

        # Set the camera
        gluLookAt(xeye, yeye, zeye, 0.0, 0.0, 0.0, upx, upy, upz)
    else:
...
def keyboard_key_callback(window, key, scancode, action, mods):
    ...
    if key == GLFW_KEY_ESCAPE and action == GLFW_PRESS:
        ...
        camera_mode = not camera_mode  # Toggle between camera and object rotation
...
def mouse_motion_callback(window, x_pos, y_pos):
    ...
    if left_mouse_button_pressed:
        ...
        theta = fmod(theta + delta_x * pix2angle_x * 0.1, 360.0)
        phi = fmod(phi + delta_y * pix2angle_y * 0.1, 360.0)  # No clamping

    if right_mouse_button_pressed:
        # Update R (zoom in/out)
        R -= delta_y * 0.1
        R = max(2.0, min(50.0, R))  # Clamp zoom range
