...
uniform float time; // Time uniform for spinning

out vec4 vertex_color;

// Pseudorandom function for fixed deformation
float rand(float seed) {
    return fract(sin(seed) * 43758.5453123);
}

void main(void) {
    // One-time deformation based on gl_InstanceID and gl_VertexID
    float deform = rand(float(gl_InstanceID) + float(gl_VertexID) * 0.1);
    vec4 deformed_position = position + vec4(deform * 0.1, deform * 0.1, deform * 0.1, 0.0);
    ...
    float angle = time + float(gl_InstanceID) * 0.2; // Vary rotation by instance ID and time
    ...
    gl_Position = P_matrix * V_matrix * M_matrix * deformed_position;
