...
light_radius = 10.0  # Promień ruchu światła
...
def render(time):
    ...
    # Ograniczanie wartości phi do zakresu [0, 180]
    phi = max(0.0, min(180.0, phi))

    # Obliczanie pozycji światła
    x = light_radius * math.sin(math.radians(phi)) * math.cos(math.radians(theta))
    y = light_radius * math.cos(math.radians(phi))
    z = light_radius * math.sin(math.radians(phi)) * math.sin(math.radians(theta))

    light_position = [x, y, z, 1.0]
    glLightfv(GL_LIGHT0, GL_POSITION, light_position)

    # Wizualizacja źródła światła jako sfera z linii
    glPushMatrix()
    glTranslatef(x, y, z)
