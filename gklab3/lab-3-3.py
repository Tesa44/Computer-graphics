...
def render(time):
    ...
    axes()  # Rysowanie osi
    # Rysowanie modelu jajka za pomocą GL_TRIANGLES
    glBegin(GL_TRIANGLES)
    for i in range(N - 1):
        for j in range(N - 1):
            # Pierwszy trójkąt
            glColor3f(*colors[i][j])  # Ustawienie koloru dla wierzchołka 1
            glVertex3f(*points[i][j])  # Wierzchołek 1

            glColor3f(*colors[i + 1][j])  # Ustawienie koloru dla wierzchołka 2
            glVertex3f(*points[i + 1][j])  # Wierzchołek 2
            
            glColor3f(*colors[i][j + 1])  # Ustawienie koloru dla wierzchołka 3
            glVertex3f(*points[i][j + 1])  # Wierzchołek 3

            # Drugi trójkąt (dopełniający prostokąt)
            glColor3f(*colors[i + 1][j])  # Ustawienie koloru dla wierzchołka 1
            glVertex3f(*points[i + 1][j])  # Wierzchołek 1

            glColor3f(*colors[i + 1][j + 1])  # Ustawienie koloru dla wierzchołka 2
            glVertex3f(*points[i + 1][j + 1])  # Wierzchołek 2
